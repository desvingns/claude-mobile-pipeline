...
- `${CLAUDE_PLUGIN_ROOT}/scripts/mp-deliver-telegram.sh [<artifact-path>]` — send a built artifact to
  yourself over Telegram (MTProto user session); emits one JSON line. Used by `--deliver`.
- `${CLAUDE_PLUGIN_ROOT}/scripts/mp-risk-route.sh --task <feature|bugfix> --spec <file> ...` —
  risk/model/quality routing; emits one JSON line. Invalid output takes the documented safe route.
- `${CLAUDE_PLUGIN_ROOT}/scripts/mp-brain-memory.sh append-candidate ...` — inbox-only durable
...
| `mp-semantic-reviewer-android` | JSON `{"pass": bool, "risk":"...", "findings":[...], "uncertainties":[...], "confidence": number}` |
