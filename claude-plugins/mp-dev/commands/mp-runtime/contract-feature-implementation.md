...
```bash
# Add --warn-only when .claude/mp/config.json sets "reviewerMode": "warn-only".
bash ${CLAUDE_PLUGIN_ROOT}/scripts/mp-reviewer-<platform>.sh [--warn-only] [each changed_file from developer JSON, space-separated]
```
...
`--warn-only` (set `"reviewerMode": "warn-only"` in `.claude/mp/config.json`), which a
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/scripts/mp-runner-<platform>.sh --scope "<modules touched by CHANGED_FILES>"
```
