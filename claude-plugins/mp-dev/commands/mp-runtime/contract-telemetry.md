...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/scripts/mp-record-run.sh --agent <step> --verdict pass|fail|partial \
...
"N runs since the last retro — run `bash ${CLAUDE_PLUGIN_ROOT}/scripts/mp-retro.sh` now? (y/N)".
