...
```bash
bash $MP_SCRIPTS/mp-runner-<platform>.sh false
```
...
Then re-run `$MP_SCRIPTS/mp-runner-<platform>.sh false`. If still `pass=false` → stop, show failures to user.
...
**Step 5 — Docs** (skip entirely when `.claude/mp/config.json` sets `"docsAgent": "inert"`):
