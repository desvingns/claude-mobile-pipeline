...
The single re-entry point: the user types `/mp --continue` instead of remembering the
...
1. Active SPEC exists → `/mp --feature --next` (resume it).
2. Active phase has unchecked tasks → `/mp --phase`.
3. A phase just completed (zero unchecked) but its row isn't `done` → `/mp --check`,
   then advance the row.
4. All phases done + clone + fit pending/divergent → `/mp --fit`.
5. Backlog non-empty → `/mp --feature --next`.
6. Fit clean + backlog empty + phases done → say the conveyor is drained; suggest
   `/mp --spec` / `--feature <idea>` (and surface the secondary signals).
