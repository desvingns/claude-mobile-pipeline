...
bash $MP_SCRIPTS/mp-reviewer-<platform>.sh [--warn-only] [each changed_file from developer JSON, space-separated]
```
...
```bash
bash $MP_SCRIPTS/mp-runner-<platform>.sh --scope "<modules touched by CHANGED_FILES>"
```
