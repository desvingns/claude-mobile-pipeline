...
for f in "${EXISTING[@]:-}"; do
  ...
  while IFS=: read -r line content; do
    ...
    prev=""
    [ "$prev_no" -ge 1 ] && prev=$(sed -n "${prev_no}p" "$f")
    printf '%s' "$prev" | grep -qE "//[[:space:]]*mp-real-io:[[:space:]]*[^[:space:]]" && continue
    offending=$(printf '%s' "$content" | sed 's/^[[:space:]]*//')
    add_v "test-clock" "$f:$line — runBlocking without a '// mp-real-io: <reason>' marker (use runTest, or justify real time): $offending"
