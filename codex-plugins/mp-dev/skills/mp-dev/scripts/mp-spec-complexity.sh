#!/usr/bin/env bash
# mp-spec-complexity.sh — deterministic size gate for a SPEC, run BEFORE
# any implementation agent. Emits exactly one JSON line; it never edits anything.
#
# Usage: mp-spec-complexity.sh --spec <file> [--cell-budget <N>]
#                                      [--freeze <path>]
#
...
if [ -n "$freeze" ] && [ "$dims" -gt 0 ]; then
  ...
  mkdir -p "$freeze_dir" 2>/dev/null || true
  {
    ...
    printf 'Generated by mp-spec-complexity.sh. %s cells across %s dimension(s).\n' "$cells" "$dims"
