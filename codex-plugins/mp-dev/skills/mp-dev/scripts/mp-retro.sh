#!/usr/bin/env bash
# mp-retro.sh — L2 per-project retro: aggregate selfimprove/runs/*.jsonl into
...
#
# Usage: mp-retro.sh [--root <repo-root>] [--health] [--stale-days N]
set -u
...

{
  ...
  echo "(\`selfimprove/lessons.md\`) or plugin improvements (\`/mp --improve\`); raw telemetry"
