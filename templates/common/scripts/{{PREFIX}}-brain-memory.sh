#!/usr/bin/env bash
# {{PREFIX}}-brain-memory.sh — safe gateway between a generated /{{PREFIX}} pipeline and
...
cmd_append_candidate() { # fire-and-forget: all outcomes are one JSON line + exit 0
  local kind="" project="" text_value="" evidence="" scope="" suggested="" source="{{PREFIX}}-knowledge"
