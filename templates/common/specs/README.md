...
- `/{{PREFIX}} --feature --next --chain` — Codex-only backlog conveyor. It chooses the same SPEC as
  `--next`; after a successful `done` transition it opens one fresh local Codex task whose only
  initial prompt is the chain command for the next runnable SPEC. It never inherits the previous
  task, creates a worktree, or creates a Git branch, and it stops cleanly when the board is drained
  or any gate blocks.
...
| `--feature --next --chain` (Codex) | same selection as `--next`; after a successful close, create one new local Codex task with no inherited conversation and the chain command as its only initial prompt, only when another runnable SPEC remains |
