...
```bash
bash "{{AGENT_DIR}}/scripts/{{PREFIX}}-spec-complexity.sh" \
  --spec "$SPEC_FILE" \
  --freeze ".ai/local/{{PREFIX}}-matrix-<spec-slug>.md"
```
...
- **`split_recommended`** — stop before the first Developer call. Show the cell count and the
  ...
  `{{PREFIX}}-planner` for a split and re-run this gate on the first resulting SPEC. On `n`,
...
`{{PREFIX}}-semantic-reviewer-android` with the approved SPEC file, the frozen acceptance matrix
...
7. **Budget: two repair cycles.** If a third semantic pass still returns blockers, stop patching.
   Surface the accumulated finding IDs and the uncovered cells, then spawn `{{PREFIX}}-architect`
