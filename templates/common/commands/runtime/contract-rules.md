...
- The **cross-project user profile** resolved by `{{PREFIX}}-brain-memory.sh` at Startup is read-only curated context and only ever BIASES recommended answers — it never auto-decides, and its absence changes nothing. `{{PREFIX}}-knowledge` and the gated `--fit` taste journal route preference candidates through the same gateway's `append-candidate` command into the brain inbox/operational layer; they never write `brain/core/*` or the curated profile directly.
