...
**Mode select (read first).** `--chain` is valid only with the exact selector
`--feature --next --chain`; reject every other use before inspecting the board. If `--feature`
carries `--next` (optionally `--chain`) or `--backlog <slug>` (or has no description while
`.claude/specs/active/` holds a SPEC) → this is **backlog-consume mode**: the SPEC already exists and
was approved when it entered the backlog, so SKIP Phase 0 + Phase 1 (no brainstorm, no questions, no
SPEC re-draft, no approval gate) and jump to Phase 2:
...
2. **Staleness pre-check (cheap, before promoting).** A backlog SPEC can be drafted well before it is consumed, and the codebase may have moved on — a prior epic/review pass, or an adjacent SPEC, can leave this one fully or partially built already. Before moving the file, do a cheap grep/explore pass for the SPEC's target symbols/screens (the `CHANGED_HINT` entries + key `WHAT` behaviour terms) against current code:
   - **Substantially already exists** → do NOT silently spawn Phase 2, but do not ask the user to confirm a fact the verification can establish. Run a deterministic targeted check: map the SPEC's acceptance obligations to the found symbols/tests/commit evidence and run the narrowest relevant check available (never a full unrelated suite). If the evidence and check pass, auto-close it: move the file straight `backlog/ → done/`, fill `Implementation links`, set `Status: done`, record `staleness_auto_closed=1`, and run **Epic completion** if this closes the epic. This verified auto-close is not a human gate and must never render an already-delivered confirmation question. If evidence is incomplete or the targeted check fails, surface the exact gap; under `--unattended`, proceed to Phase 2 as the recommended non-destructive default and record `staleness_auto_proceeded=1`, otherwise ask whether to proceed. Do not claim delivery.
   - **Partially exists** → surface the gap (built vs. still-needed). With `--unattended` (including the equivalent natural-language instruction to skip human gates), auto-accept the recommended re-scope, record `staleness_auto_rescoped=1`, and tighten `WHAT`/`CONSTRAINTS` to the actual remaining gap before Phase 2. Otherwise offer "Re-scope the SPEC to the remaining gap before implementing? (y/N)" — on `y`, tighten the same approved file; on `N`, proceed with the SPEC as written.
